...
if user_menu == "Medal Tally":
    ...
    if selected_year != "Overall" and selected_country == "Overall":
        st.title("Overall Tally in " + str(selected_year) + " Olympics")
    if selected_year == 'Overall' and selected_country != 'Overall':
...
if user_menu == 'Overall Analysis':
    ...
    st.title("Participating Athlete over the year")
    st.plotly_chart(fig)
...
if user_menu == 'Country-wise Analysis':
    ...
    country_list.sort()
    selected_country = st.sidebar.selectbox('Select the Country Name', country_list)
    ...
    st.title("Top 10 Athlete in "+ selected_country)
    top10_athlete = helper.most_successful_countrywise(df,selected_country)
    st.table(top10_athlete)
...
if user_menu == 'Athlete wise Analysis':
    ...
    fig = ff.create_distplot([x1, x2, x3, x4],group_labels=['Overall Age', 'Gold Medalist', 'Silver Medalist', 'Bronze Medalist'],show_hist=False, show_rug=False)
    ...
    fig, ax = plt.subplots()
    ax = sns.scatterplot(temp_df, x='Weight', y='Height', hue = 'Medal', style='Sex', s=100)
    ...
    famous_sport = ['Basketball','Judo', 'Football','Tug-Of-War', 'Athletics',
                    ...
                    'Cycling', 'Diving', 'Canoeing', 'Tennis', 'Golf', 'Softball', 'Archery',
                    'Volleyball', 'Synchronized Swimming', 'Rugby Sevens', 'Beach Volleyball', 'Table Tennis', 'Baseball',
                    'Rhythmic Gymnastics', 'Triathlon', 'Polo', 'Ice Hockey']
